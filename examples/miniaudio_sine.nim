...
proc audioCallback(pDevice: ptr ma_device, pOutput: pointer, pInput: pointer, frameCount: ma_uint32) {.cdecl.} =
    let
...
proc main() =
  var
    res: ma_result
    context: ma_context
    backends: array[3, ma_backend] = [ma_backend_jack, ma_backend_pulseaudio, ma_backend_alsa]
